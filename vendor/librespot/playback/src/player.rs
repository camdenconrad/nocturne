        ...
        match self.sink_status {
            SinkStatus::Running => {
                ...
                match self.sink.stop() {
                    ...
                    Err(e) => {
                        // NOCTURNE PATCH: upstream calls exit(1) here, which kills the whole
                        // process when a sink stop/drain fails — e.g. when the audio daemon
                        // (runic) restarts under a live stream and the auto-pause path drains a
                        // dead connection. The device is gone; there is nothing to drain. Treat
                        // the sink as closed and keep running — the next play rebuilds a fresh
                        // sink via the SinkBuilder.
                        error!("sink stop failed (treating sink as closed): {e}");
                        self.sink_status = if temporarily {
                            SinkStatus::TemporarilyClosed
                        } else {
                            SinkStatus::Closed
